...
import { 
  Utensils, Sparkles, ShieldAlert, Activity, Timer, Mic, Music, Play
} from 'lucide-react';
...
export const EventOverlay: React.FC<EventOverlayProps> = ({
  ...
  // Reset timer states when event changes
  useEffect(() => {
    setPressureTimer(10);
  ...
  const renderStory = () => {
    ...
    if (isCutlery) {
        ...
        description = "Se han localizado cubiertos sucios en la cocina. Como nadie asume la responsabilidad, el LAB identificará a un responsable. Si no supera el reto, perderá 1 crédito.";
        icon = <Utensils size={40} />;
        colorClass = "bg-orange-500 border-orange-400";
    } else if (isRobbery) {
        title = "Brecha de Seguridad";
        description = "Alguien ha dejado la oficina abierta. Unos intrusos han sustraído 1 crédito de cada persona. Hay una única oportunidad de recuperarlos colectivamente.";
        icon = <ShieldAlert size={40} />;
        colorClass = "bg-red-600 border-red-500";
    } else if (isPressure) {
        title = "¡Bajo presión!";
        description = "Se elegirá aleatoriamente a un participante que tendrá 10 segundos para enumerar 5 productos de Niche Beauty Lab. Si no lo cumple será castigado con 1 crédito.";
        icon = <Timer size={40} />;
        colorClass = "bg-blue-600 border-blue-500";
    } else if (isSinging) {
        title = "Te toca dar el cante";
        description = "Se elegirá aleatoriamente a un participante que tendrá que cantar un villancico. Si no supera la prueba será castigado con 1 crédito.";
        icon = <Mic size={40} />;
    ...
    return (
      <div className="animate-in fade-in slide-in-from-bottom-12 duration-500 max-w-3xl w-full text-center px-4 md:px-6">
        ...
        {isAdmin && (
          ...
          >
            {isRobbery ? 'Iniciar Recuperación' : 'Identificar Sujeto'}
          </button>
        )}
      </div>
    );
  ...
  const renderAction = () => {
    ...
    if (isCutlery) actionInstruction = "RETO: 3 FLEXIONES";
            ...
            <div className="animate-in zoom-in duration-500 w-full max-w-xl">
               <div className={`bg-[#18181b] border-4 ${colorTheme.border} p-8 md:p-12 rounded-[40px] md:rounded-[50px] mb-8 md:mb-12 relative overflow-hidden shadow-2xl`}>
                 ...
                 <p className={`${colorTheme.accent} font-black uppercase tracking-[0.4em] md:tracking-[0.5em] text-[10px] md:text-xs mb-4 md:mb-6`}>
                     {isRobbery ? "Protocolo de Emergencia" : "Sujeto Seleccionado"}
                 </p>
                 ...
                 <p className="text-white/40 text-[9px] md:text-[11px] font-bold uppercase leading-relaxed tracking-widest mt-4">
                     {isRobbery 
                        ? "Todo el personal debe coordinarse. Si fallan, restaremos 1 crédito a todos." 
                        : "Si el sujeto no supera la prueba, restaremos 1 crédito de su terminal (siempre que tenga créditos disponibles)."}
                 </p>
               </div>
